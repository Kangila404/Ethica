...
import {
  ...
  type PhilosopherRepository,
} from '../domain/repository/philosopher.repository';
import {
  USER_REPOSITORY,
  type UserRepository,
...
} from '../domain/repository/user-philosopher-count.repository';
import {
  POST_REPOSITORY,
  type PostRepository,
} from '../domain/repository/post.repository';
import {
  POST_SEGMENT_REPOSITORY,
  type PostSegmentRepository,
} from '../domain/repository/post-segment.repository';
      ...
      totalCount === 0
        ...
        : all
            ...
            .map((item) => ({
              ...
              closenessPercent:
                Math.round(
                  ((countById.get(item.philosopher.id) ?? 0) / totalCount) *
                    1000,
                ) / 10,
            }));
