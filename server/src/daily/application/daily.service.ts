...
export class DailyService {
    ...
    async getDaily(userId: string): Promise<DailyQuestionResponse> {
        ...
        await this.validationUserStatus(user.userStatus);

        const today = this.getTodayString();
        const userDailyQuestion = await this.userDailyQuestionRepository
            .findByUserIdAndServiceDate(user.id, today);
        if (!userDailyQuestion) {
            throw new NotFoundException('오늘의 질문을 찾을 수 없습니다.');
        }

        const question = await this.questionRepository
            .findByIdWithAnswers(userDailyQuestion.questionId);
        if (!question) {
    ...
    async assignForToday(userId: string): Promise<void> {
        ...
        // 1. 멱등 가드
        const existing = await this.userDailyQuestionRepository
            .findByUserIdAndServiceDate(user.id, today);
        if (existing) return;

        // 2. 안 받은 daily 문제 하나
        const servedIds = await this.userDailyQuestionRepository
            .findServicedQuestionIds(user.id);
        const question = await this.questionRepository
            .findRandomDailyExcluding(servedIds);
    ...
    async submitStageTwo(userId:string, request:SubmitStageTwoRequest):Promise<SubmitStageTwoResponse>{
        const user = await this.findUserByUserIdOrThrow(userId);
        this.validationUserStatus(user.userStatus);
        const followupAnswer = await this.findFollowupAnswerByIdOrThrow(request.followupAnswerId);
        const userFollowupAnswer = UserFollowupAnswer.createUserFollowupAnswer(user.id, (await followupAnswer).id);
        await this.userFollowupAnswerRepository.save(userFollowupAnswer);
        return SubmitStageTwoResponse.from(followupAnswer);
    }


    async submitStageOne(userId:string, request:SubmitStageOneRequest):Promise<SubmitStageOneResponse>{
        const user = await this.findUserByUserIdOrThrow(userId);
        const today = this.getTodayString();
        // 오늘의 문제
        const dailyQuestion = await this.userDailyQuestionRepository.findByUserIdAndServiceDate(user.id, today);
        // 제출된 문제
        ...
        // 이미 푼 문제인지 확인
        await this.validationSubmittedAnswer(dailyQuestion.status);

        const question = await this.findQuestionByIdOrThrow(dailyQuestion.questionId);
        ...
        // 유저 대답 생성
        const userAnswer = UserAnswer.createUserStageOneAnswer(user.id, request.answerId);
    ...
    async findFollowupAnswerByIdOrThrow(followupAnswerId:string):Promise<FollowupAnswer>{
        const followupAnswer = await this.followupAnswerRepository.findById(followupAnswerId);
        if(!followupAnswer){
    ...
    // 검증 메서드
    async validationUserStatus(userStatus: UserStatus): Promise<void> {
        if (userStatus !== UserStatus.ACTIVE) {
            throw new ForbiddenException('정지된 계정입니다.');
        }
    }

    async validationSubmittedAnswer(dailyQuestionStatus:DailyQuestionStatus):Promise<void>{
