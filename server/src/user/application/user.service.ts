...
export class UserService {
    ...
    async updateNickname(userId: string, request: NicknameUpdateRequest): Promise<MessageResponse> {
        const user = await this.getUserOrThrow(userId);
    ...
    async updateDailyTime(userId: string, request: DailyTimeRequest): Promise<MessageResponse> {
        const user = await this.getUserOrThrow(userId);
    ...
    async updateNotification(userId:string, request:NotificationRequest): Promise<NotificationResponse>{
        const user = await this.getUserOrThrow(userId);
